...
Mesa::Mesa(int nRows, int nCols)
 : m_rows(nRows), m_cols(nCols), m_player(nullptr), m_nGarks(0), m_history(nRows, nCols)//History object takes in 2 int parameters
{
...
bool Mesa::attackGarkAt(int r, int c, int dir)
{
    ...
    if(k < m_nGarks)
        m_history.record(m_player -> row(), m_player -> col()); //should be put before return statements; records position of player when attacking
...
//return a reference to Mesa's History object
