    ...
    while ( ! player->isDead()  &&  m_mesa->garkCount() > 0)
    {
        ...
        else
        {
            switch (action[0])
            {
                ...
                case 'q':
                    return;
                case 'h': //add a case for if h is entered, show history
                    m_mesa -> history().display(); //history() returns ref to History object, call display()
        ...
        //m_mesa->moveGarks(); moved to line 93. If left here, garks will move after viewing history which is unwanted
        m_mesa->display();
    }
}
